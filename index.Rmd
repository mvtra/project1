...
opioid_overdose %>% group_by(Opioid.Dispensing.Rate.per.100>50) %>% pivot_longer(contains("50"), values_to="Opioid.Dispensing.Rate.per.100>50") %>% select(-name)

opioid_overdose %>% group_by(Opioid.Dispensing.Rate.per.100>50) %>% summarize(mean=mean(Number.of.Deaths),sd=sd(Number.of.Deaths)) %>% pivot_longer(-1, values_to='Number of Deaths')
